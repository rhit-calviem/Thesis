...
class OSAG(nn.Module):
    ...
    def __init__(self, channels, depth = 2):
        ...
        # INCREASED DEPTH: Stacking Meso and Global blocks
        self.meso_layers = nn.ModuleList([MesoOSABlock(channels) for _ in range(depth)])
        self.glob_layers = nn.ModuleList([GlobalOSABlock(channels) for _ in range(depth)])
    ...
    def forward(self, x):
        ...
        # out = self.meso(out)
        # res = self.glob(out)
        for meso in self.meso_layers:
            out = meso(out)
        for glob in self.glob_layers:
            out = glob(out)
        # fused = res + identity
        fused = out + identity
