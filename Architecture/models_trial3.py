...
#   4. Inverse rotation (R^{-1}) to restore shape (Eq. 5)
    ...
    def __init__(self, dim, num_heads, shared_qkv=None):  # accept shared_qkv
        super().__init__()
        self.dim = dim
        ...
        # Use shared projection if provided, otherwise create own
        self.qkv = shared_qkv if shared_qkv is not None else nn.Linear(dim, dim * 3)
...
class MesoOSABlock(nn.Module):
    ...
    def __init__(self, dim, window_size=8, num_heads=4, shared_qkv=None):  # add shared_qkv
        super().__init__()
        self.dim = dim
        self.window_size = window_size
        self.norm1 = nn.LayerNorm(dim)
        self.osa = OSA(dim, num_heads, shared_qkv=shared_qkv)  # pass it down
...
class GlobalOSABlock(nn.Module):
    def __init__(self, dim, grid_size=8, num_heads=4, shared_qkv=None):  # add shared_qkv
        super().__init__()
        self.dim = dim
...
class OSAG(nn.Module):
    ...
    def __init__(self, channels):
        ...
        # Single shared QKV projection for both Meso and Global
        self.shared_qkv = nn.Linear(channels, channels * 3)

        self.lcb = LocalConvBlock(channels)
        self.meso = MesoOSABlock(channels, shared_qkv=self.shared_qkv)  # pass in
        self.glob = GlobalOSABlock(channels, shared_qkv=self.shared_qkv)  # pass in
